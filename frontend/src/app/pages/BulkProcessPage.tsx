...
export const BulkProcessPage = () => {
  ...
  const processEmails = async () => {
    ...
    const catchAllCount = emailResults.filter(
      (r) => r.status === "catch-all",
    ).length;
    const invalidCount = emailResults.filter(
      (r) => r.status === "invalid",
    ).length;
    const riskyCount = emailResults.filter((r) => r.status === "risky").length;
    const unknownCount = emailResults.filter(
      (r) => r.status === "unknown",
    ).length;
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4">
            ...
            <div className="text-center p-4 rounded-lg bg-accent/30">
              <div className="text-2xl font-bold text-gray-900">
                {results.length > 0
                  ? ((validCount / results.length) * 100).toFixed(0)
                  : 0}
                %
              </div>
