...
export const AdminDashboard = () => {
  ...
  const validCount = allVerifications.filter(
    (v) => v.status === "valid",
  ).length;
  const catchAllCount = allVerifications.filter(
    (v) => v.status === "catch-all",
  ).length;
  const invalidCount = allVerifications.filter(
    (v) => v.status === "invalid",
  ).length;
  const riskyCount = allVerifications.filter(
    (v) => v.status === "risky",
  ).length;
  ...
  const planDistribution = [
    {
      plan: "Free",
      count: allUsers.filter((u) => u.plan === "free").length,
      color: "#6B7280",
    },
    {
      plan: "Business",
      count: allUsers.filter((u) => u.plan === "business").length,
      color: "#2563EB",
    },
    {
      plan: "Enterprise",
      count: allUsers.filter((u) => u.plan === "enterprise").length,
      color: "#1E3A8A",
    },
  ];
          ...
          <CardContent>
            <div className="space-y-3">
              {allUsers.slice(0, 5).map((user) => (
                <div
                  key={user.id}
                  className="flex items-center justify-between p-3 sm:p-4 rounded-xl border border-[#F1F5F9] bg-white hover:shadow-md transition-all gap-3"
                >
                  ...
                  <div className="text-right shrink-0">
                    <div
                      className={`text-xs font-bold uppercase tracking-wider px-2 py-1 rounded-md mb-1 ${
